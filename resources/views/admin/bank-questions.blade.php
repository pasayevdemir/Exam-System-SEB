...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCheckboxBehavior() {
        if (singleRadio.checked) {
            ...
            correctAnswers.forEach(checkbox => {
                ...
                // Clear all selections when switching to single choice
        ...
        // Update the label text to indicate the behavior
        const helpText = document.getElementById('correct-answers-help');
        if (helpText) {
            helpText.remove();
        }

        const container = document.getElementById('mcqAnswersContainer');
        const newHelpText = document.createElement('small');
        newHelpText.id = 'correct-answers-help';
        newHelpText.className = 'text-muted';

        if (singleRadio.checked) {
            newHelpText.textContent = 'Select ONE correct answer';
        } else if (multipleRadio.checked) {
            newHelpText.textContent = 'Select ALL correct answers (you can select multiple options)';
        }
    ...
    // Handle dynamic required attributes for answer inputs
    function updateAnswerRequirements() {
        ...
        // Ensure at least 2 answers are required
        answerInputs.forEach((input, index) => {
            if (index < Math.max(2, filledCount)) {
                input.setAttribute('required', 'required');
    ...
    document.querySelectorAll('.answer-text-input').forEach(input => {
        input.addEventListener('input', updateAnswerRequirements);
        input.addEventListener('blur', updateAnswerRequirements);
    });
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        if (!fileUploadRadio.checked) {
            const checkedCorrectAnswers = document.querySelectorAll('.correct-answer:checked');
            const filledAnswers = document.querySelectorAll('.answer-text-input');
            const filledAnswersArray = Array.from(filledAnswers).filter(input => input.value.trim() !== '');
            const filledAnswersCount = filledAnswersArray.length;
            ...
            if (checkedCorrectAnswers.length === 0) {
                e.preventDefault();
                alert('Please select at least one correct answer for your question.');
                return false;
            }

            // Check if selected correct answers correspond to filled answer fields
            let validCorrectAnswers = 0;
            checkedCorrectAnswers.forEach(checkbox => {
                const index = parseInt(checkbox.value);
                const correspondingInput = filledAnswers[index];
                if (correspondingInput && correspondingInput.value.trim() !== '') {
                    validCorrectAnswers++;
                }
            });

            if (validCorrectAnswers === 0) {
                e.preventDefault();
                alert('Please make sure you have marked at least one filled answer option as correct.');
                return false;
            }

            // Filter out empty answers before form submission
            filledAnswers.forEach((input, index) => {
                if (input.value.trim() === '') {
                    input.disabled = true;
                    // Also disable corresponding correct answer checkbox if it's not checked
                    const checkbox = document.querySelector(`.correct-answer[value="${index}"]`);
                    if (checkbox) {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const questionText = this.closest('.border').querySelector('p').textContent.trim();

            // Update modal content
            questionToDelete.textContent = `Question ${questionNumber}: ${questionText.substring(0, 100)}${questionText.length > 100 ? '...' : ''}`;

            // Update form action
            deleteForm.action = `/examadmin/question/${questionId}`;
